...
export function StreamLaunchPanel({ user, script, productName, productImageUrl, hostName, layout }: Props) {
  ...
  async function launchBroadcast() {
    ...
    try {
      ...
      const response = await fetch("/api/broadcast/start", {
        ...
        body: JSON.stringify({ destinationId, presenterJobId, scenePlan, productImageUrl }),
      });
      const result = await response.json();
